...
function init() {
    ...
    if(config.flush_policy=='time') {
        instent_flush_fname = config.flush_dir + '\/_ast_'+ (new Date().getTime()) +'.fsh';
    ...
    if(config.flush_policy=='time' || config.flush_policy=='always') {
        var files = fs.readdirSync(config.flush_dir);
        var fsh_file_max = 0;
        var fsh_file = '';
        for(i=0; i<files.length; i++) {
            if(files[i].match('_as') && files[i].match('.fsh')) {
                if(files[i].split('_')[2]!=null && files[i].split('_')[2].split('.')[0]!=null 
                    && Number(files[i].split('_')[2].split('.')[0])>fsh_file_max) {
                    if(instent_flush_fname.match(files[i])) continue;
                    fsh_file_max = Number(files[i].split('_')[2].split('.')[0]);
                    fsh_file = files[i];
                 }
            }
        }
        if(fsh_file_max!=0) {
            disk.readAll(config.flush_dir + '\/'+ fsh_file, {}, function(err, _item_max_size, _max_item_number, _item_sizes, _big_buffer, _item_number, _item_index, _empty_index){
                if(err) return client.write('{error:41, message:"err to restore form disk"}\r\n');
                ...
                exec('rm -f ' + config.flush_dir + '\/'+ fsh_file);
            });
        }
    }
}
...
{
    if(fname==null) fname = instent_flush_fname;
