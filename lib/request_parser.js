...
{
	var fpath = config.flush_dir + '\/'+ (new Date().getTime()) +'.flush';
	disk.flushAll(fpath, {item_max_size: config.item_max_size, item_number:item_number, item_sizes:item_sizes}, big_buffer, function(err, written){
		if(err) return client.write('{error:31, message:"flush all data error"}\r\n');
		client.client.write(fpath+'\r\n');
	});
}
